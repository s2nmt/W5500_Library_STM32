...
void MQTTClientInit(Client* c, Network* network, unsigned int command_timeout_ms,
...
int MQTTPublish(Client* c, const char* topicName, MQTTMessage* message)
{
    ...
    if (len <= 0) goto exit;
    if ((rc = sendPacket(c, len, &timer)) != SUCCESSS) goto exit; // there was a problem
